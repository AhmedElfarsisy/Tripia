...
public class RegisterViewModel extends ViewModel implements FirebaseDelegate, FacebookDelegate, GoogleDelegate {
    public MutableLiveData<Boolean> doSignup = new MutableLiveData<>();
    public MutableLiveData<Boolean> isSignedupSuccessed = new MutableLiveData<>();
    public MutableLiveData<String> isSignedupFailure = new MutableLiveData<>();
    public MutableLiveData<Boolean> isSignedupCanceled = new MutableLiveData<>();
    ...
    public RegisterViewModel(IFirebaseRepo firebaseRepo, IFacebookRepo facebookRepo, IGoogleRepo googleRepo) {
    ...
    public void signupUser(String email, String password) {
        if (Validations.isValidEmail(email)) {
            if (Validations.isValidPassword(password)) {
                firebaseRepo.registerWithFirebase(email, password);
            } else {
    ...
    public void onFacebookRegistrationCancel() {
        isSocialCanceled.setValue(true);
    }

    @Override
    public void onFacebookRegistrationError(FacebookException error) {
        isSocialFailure.setValue(error.getMessage());
    }
    ...
    public void onHandleFacebookTokenFailure(Exception exception) {
        isSocialFailure.setValue(exception.getMessage());
    }
    ...
    private void initBooleanValues() {
        doSignup.setValue(false);
        isSignedupSuccessed.setValue(false);
        isSignedupFailure.setValue("");
        isSignedupCanceled.setValue(false);
